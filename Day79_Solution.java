class Solution {
    ...
    static boolean dfs(char[][] mat, String word, int i, int j, int index, boolean[][] visited){
        ...
        visited[i][j] = true;
        boolean found = dfs(mat, word, i - 1, j, index + 1, visited) || dfs(mat, word, i + 1, j, index + 1, visited) || dfs(mat, word, i, j - 1, index + 1, visited) || dfs(mat, word, i, j + 1, index + 1, visited);
        visited[i][j] = false;
        return found;
    }
}